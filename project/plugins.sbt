addSbtPlugin("africa.shuwari" % "sbt-snx" % "0.2.0")
addSbtPlugin("africa.shuwari" % "sbt-version" % "0.7.0")
addSbtPlugin("ch.epfl.scala" % "sbt-bloop" % "2.1.0")
addSbtPlugin("ch.epfl.scala" % "sbt-github-dependency-submission" % "3.2.3")
addSbtPlugin("ch.epfl.scala" % "sbt-scalafix" % "0.14.7")
addSbtPlugin("com.alejandrohdezma" % "sbt-mdoc-toc" % "0.5.0")
addSbtPlugin("com.disneystreaming.smithy4s" % "smithy4s-sbt-codegen" % "0.19.7")
addSbtPlugin("com.earldouglas" % "sbt-war" % "5.2.1")
addSbtPlugin("com.eed3si9n" % "sbt-assembly" % "2.3.1")
addSbtPlugin("com.eed3si9n" % "sbt-buildinfo" % "0.13.1")
addSbtPlugin("com.eed3si9n" % "sbt-nocomma" % "0.1.3")
addSbtPlugin("com.eed3si9n" % "sbt-vimquit" % "0.1.1")
addSbtPlugin("com.evolution" % "sbt-dependency-rules-plugin" % "1.0.0")
addSbtPlugin("com.evolution" % "sbt-kagi-plugin" % "0.1.0")
addSbtPlugin("com.github.ghostdogpr" % "caliban-codegen-sbt" % "3.1.2")
addSbtPlugin("com.github.reibitto" % "sbt-test-shards" % "0.2.1")
addSbtPlugin("com.github.sbt" % "flyway-sbt" % "12.0.0")
addSbtPlugin("com.github.sbt" % "sbt-boilerplate" % "0.8.0")
addSbtPlugin("com.github.sbt" % "sbt-ci-release" % "1.11.2")
addSbtPlugin("com.github.sbt" % "sbt-coffeescript" % "2.1.0-M3")
addSbtPlugin("com.github.sbt" % "sbt-concat" % "1.1.0-M1")
addSbtPlugin("com.github.sbt" % "sbt-digest" % "2.2.0-M1")
addSbtPlugin("com.github.sbt" % "sbt-dynver" % "5.1.1")
addSbtPlugin("com.github.sbt" % "sbt-eclipse" % "6.3.0-M1")
addSbtPlugin("com.github.sbt" % "sbt-git" % "2.1.0")
addSbtPlugin("com.github.sbt" % "sbt-gzip" % "2.1.0-M1")
addSbtPlugin("com.github.sbt" % "sbt-header" % "5.11.0")
addSbtPlugin("com.github.sbt" % "sbt-jacoco" % "3.6.0")
addSbtPlugin("com.github.sbt" % "sbt-java-formatter" % "0.12.0")
addSbtPlugin("com.github.sbt" % "sbt-java-formatter-add-opens" % "0.11.1-M1")
addSbtPlugin("com.github.sbt" % "sbt-javaagent" % "0.2.0")
addSbtPlugin("com.github.sbt" % "sbt-js-engine" % "1.4.0-M4")
addSbtPlugin("com.github.sbt" % "sbt-jshint" % "2.1.0-M1")
addSbtPlugin("com.github.sbt" % "sbt-less" % "2.1.0-M2")
addSbtPlugin("com.github.sbt" % "sbt-license-report" % "1.9.0")
addSbtPlugin("com.github.sbt" % "sbt-maven-plugin" % "0.0.4")
addSbtPlugin("com.github.sbt" % "sbt-mocha" % "2.2.0-M3")
addSbtPlugin("com.github.sbt" % "sbt-native-packager" % "1.11.7")
addSbtPlugin("com.github.sbt" % "sbt-osgi" % "0.11.0-RC1")
addSbtPlugin("com.github.sbt" % "sbt-pgp" % "2.3.1")
addSbtPlugin("com.github.sbt" % "sbt-protobuf" % "0.8.3")
addSbtPlugin("com.github.sbt" % "sbt-release" % "1.5.0")
addSbtPlugin("com.github.sbt" % "sbt-rjs" % "2.1.0-M1")
addSbtPlugin("com.github.sbt" % "sbt-stylus" % "2.1.0-M1")
addSbtPlugin("com.github.sbt" % "sbt-unidoc" % "0.6.1")
addSbtPlugin("com.github.sbt" % "sbt-web" % "1.6.0-M4")
addSbtPlugin("com.github.sbt" % "sbt-web-build-base" % "2.1.0-M3")
addSbtPlugin("com.github.sbt" % "sbt2-compat" % "0.1.0")
addSbtPlugin("com.github.sbt.junit" % "sbt-jupiter-interface" % "0.19.0")
addSbtPlugin("com.github.scalaprops" % "sbt-scalaprops" % "0.5.3")
addSbtPlugin("com.github.xuwei-k" % "coherence-plugin" % "0.1.1")
addSbtPlugin("com.github.xuwei-k" % "given-anonymous-name-plugin" % "0.1.1")
addSbtPlugin("com.github.xuwei-k" % "junit-report-aggregate" % "0.1.2")
addSbtPlugin("com.github.xuwei-k" % "proto-unused-imports" % "0.1.3")
addSbtPlugin("com.github.xuwei-k" % "sbt-class-diagram" % "0.3.1")
addSbtPlugin("com.github.xuwei-k" % "sbt-conflict-classes" % "0.1.2")
addSbtPlugin("com.github.xuwei-k" % "sbt-jol" % "0.3.2")
addSbtPlugin("com.github.xuwei-k" % "sbt-jshell" % "0.1.5")
addSbtPlugin("com.github.xuwei-k" % "sbt-root-aggregate" % "0.1.0")
addSbtPlugin("com.github.xuwei-k" % "sbt-shapeless-generic-counter" % "0.1.2")
addSbtPlugin("com.github.xuwei-k" % "scalafix-check" % "0.1.0")
addSbtPlugin("com.github.xuwei-k" % "scalafix-rule-resource-gen" % "0.1.3")
addSbtPlugin("com.github.xuwei-k" % "test-times-plugin" % "0.1.2")
addSbtPlugin("com.github.xuwei-k" % "unused-code-plugin" % "0.5.5")
addSbtPlugin("com.github.xuwei-k" % "unused-proto-plugin" % "0.2.2")
addSbtPlugin("com.github.xuwei-k" % "warning-diff-scalafix-plugin" % "0.6.7")
addSbtPlugin("com.indoorvivants" % "bindgen-sbt-plugin" % "0.4.5")
addSbtPlugin("com.indoorvivants" % "sbt-commandmatrix" % "0.1.0")
addSbtPlugin("com.indoorvivants" % "sbt-forge-native-binary" % "0.2.0")
addSbtPlugin("com.indoorvivants" % "sbt-revolver" % "0.11.2")
addSbtPlugin("com.indoorvivants" % "sbt-revolver-core" % "0.11.2")
addSbtPlugin("com.indoorvivants" % "sbt-revolver-native" % "0.11.2")
addSbtPlugin("com.indoorvivants.snapshots" % "sbt-snapshots" % "0.0.13")
addSbtPlugin("com.jamesward" % "sbt-reload" % "0.0.2")
addSbtPlugin("com.julianpeeters" % "sbt-avrohugger" % "2.17.0")
addSbtPlugin("com.lightbend.paradox" % "sbt-paradox" % "0.11.0-M4")
addSbtPlugin("com.lightbend.paradox" % "sbt-paradox-theme" % "0.11.0-M4")
addSbtPlugin("com.performantdata" % "sbt-sliquibase" % "0.3.0")
addSbtPlugin("com.siriusxm" % "sbt-snapshot4s" % "0.2.10")
addSbtPlugin("com.skillsjars" % "skillsjars-sbt-plugin" % "0.0.9")
addSbtPlugin("com.softwaremill.sttp.tapir" % "sbt-openapi-codegen" % "1.13.21")
addSbtPlugin("com.thesamet" % "sbt-protoc" % "1.1.0-RC1")
addSbtPlugin("com.thoughtworks.sbt-api-mappings" % "sbt-api-mappings" % "3.0.3+7-b0b88794")
addSbtPlugin("com.timushev.sbt" % "sbt-updates" % "0.7.0")
addSbtPlugin("com.typesafe" % "sbt-mima-plugin" % "1.1.6")
addSbtPlugin("dev.cheleb" % "sbt-fullstack-js" % "0.8.0")
addSbtPlugin("dev.cheleb" % "sbt-plantuml" % "0.3.5")
addSbtPlugin("io.github.matejcerny" % "sbt-config" % "0.5.2")
addSbtPlugin("io.github.nestor10" % "sbt-spark-pipelines" % "0.1.0")
addSbtPlugin("io.github.olegych" % "sbt-cached-ci" % "2.0.0")
addSbtPlugin("io.github.polentino" % "sbt-redacted" % "1.1.1")
addSbtPlugin("io.github.sbt-dao-generator" % "sbt-dao-generator" % "2.1.1")
addSbtPlugin("io.github.sbt-doctest" % "sbt-doctest" % "0.12.5")
addSbtPlugin("io.github.sbt-teavm" % "sbt-teavm" % "0.5.0")
addSbtPlugin("io.github.scalate" % "sbt-scalate-precompiler" % "1.11.0.0")
addSbtPlugin("io.github.scalauser12" % "sbt-release-io" % "0.13.3")
addSbtPlugin("io.github.scalauser12" % "sbt-release-io-monorepo" % "0.13.3")
addSbtPlugin("io.h8.sbt" % "sbt-dependencies" % "1.0.4")
addSbtPlugin("io.stryker-mutator" % "sbt-stryker4s" % "0.21.0")
addSbtPlugin("me.limansky" % "sbt-test-it" % "0.3.1")
addSbtPlugin("me.seroperson" % "sbt-live-reload" % "0.1.1")
addSbtPlugin("me.seroperson" % "sbt-testkit-plugin" % "0.1.0")
addSbtPlugin("net.nmoncho" % "sbt-dependency-check" % "1.9.1")
addSbtPlugin("net.sf.ij-plugins" % "sbt-imagej" % "2.2.1")
addSbtPlugin("nl.gn0s1s" % "sbt-banner" % "0.0.2")
addSbtPlugin("nl.gn0s1s" % "sbt-dotenv" % "3.2.0")
addSbtPlugin("nl.gn0s1s" % "sbt-jackson-version-check" % "0.0.8")
addSbtPlugin("nl.gn0s1s" % "sbt-license-check" % "0.0.6")
addSbtPlugin("nl.gn0s1s" % "sbt-pekko-version-check" % "0.0.9")
addSbtPlugin("org.foundweekends.conscript" % "sbt-conscript" % "0.5.9")
addSbtPlugin("org.foundweekends.giter8" % "sbt-giter8" % "0.18.0")
addSbtPlugin("org.foundweekends.giter8" % "sbt-giter8-scaffold" % "0.18.0")
addSbtPlugin("org.jetbrains.scala" % "sbt-idea-shell" % "2025.3.0")
addSbtPlugin("org.jetbrains.scala" % "sbt-structure-extractor" % "2026.2.3")
addSbtPlugin("org.latestbit" % "sbt-gcs-plugin" % "2.0.0-RC1")
addSbtPlugin("org.playframework" % "play-docs-sbt-plugin" % "3.1.0-M9")
addSbtPlugin("org.playframework" % "sbt-play-ebean" % "9.0.0-M2")
addSbtPlugin("org.playframework" % "sbt-plugin" % "3.1.0-M9")
addSbtPlugin("org.playframework" % "sbt-scripted-tools" % "3.1.0-M9")
addSbtPlugin("org.playframework.twirl" % "sbt-twirl" % "2.1.0-M9")
addSbtPlugin("org.polyvariant" % "smithy-scala-tools-sbt" % "0.3.1")
addSbtPlugin("org.scala-native" % "sbt-scala-native" % "0.5.12")
addSbtPlugin("org.scala-sbt" % "sbt-contraband" % "0.9.0")
addSbtPlugin("org.scalameta" % "sbt-mdoc" % "2.9.0")
addSbtPlugin("org.scalameta" % "sbt-metals" % "1.6.7")
addSbtPlugin("org.scalameta" % "sbt-munit" % "1.3.3")
addSbtPlugin("org.scalameta" % "sbt-native-image" % "0.5.0")
addSbtPlugin("org.scalameta" % "sbt-scalafmt" % "2.6.1")
addSbtPlugin("org.scalaxb" % "sbt-scalaxb" % "1.12.4")
addSbtPlugin("org.scoverage" % "sbt-scoverage" % "2.4.4")
addSbtPlugin("org.typelevel" % "sbt-tpolecat" % "0.5.6")
addSbtPlugin("org.wartremover" % "sbt-wartremover" % "3.5.8")
addSbtPlugin("org.wartremover" % "sbt-wartremover-contrib" % "2.4.5")
addSbtPlugin("org.webjars" % "sbt-webjars" % "0.0.6")
addSbtPlugin("org.wvlet.uni" % "sbt-uni" % "2026.1.11")
addSbtPlugin("org.xerial.sbt" % "sbt-pack" % "1.0.0")
addSbtPlugin("pl.project13.scala" % "sbt-jmh" % "0.4.8")
