addSbtPlugin("africa.shuwari" % "sbt-version" % "0.6.2")
addSbtPlugin("ch.epfl.scala" % "sbt-bloop" % "2.0.19")
addSbtPlugin("ch.epfl.scala" % "sbt-github-dependency-submission" % "3.2.1")
addSbtPlugin("ch.epfl.scala" % "sbt-scalafix" % "0.14.6")
addSbtPlugin("com.alejandrohdezma" % "sbt-mdoc-toc" % "0.5.0")
addSbtPlugin("com.disneystreaming.smithy4s" % "smithy4s-sbt-codegen" % "0.19.0-RC1")
addSbtPlugin("com.earldouglas" % "sbt-war" % "5.1.1")
addSbtPlugin("com.eed3si9n" % "sbt-assembly" % "2.3.1")
addSbtPlugin("com.eed3si9n" % "sbt-buildinfo" % "0.13.1")
addSbtPlugin("com.eed3si9n" % "sbt-nocomma" % "0.1.3")
addSbtPlugin("com.eed3si9n" % "sbt-vimquit" % "0.1.1")
addSbtPlugin("com.evolution" % "sbt-dependency-rules-plugin" % "1.0.0")
addSbtPlugin("com.github.ghostdogpr" % "caliban-codegen-sbt" % "3.0.0")
addSbtPlugin("com.github.sbt" % "flyway-sbt" % "12.0.0")
addSbtPlugin("com.github.sbt" % "sbt-boilerplate" % "0.8.0")
addSbtPlugin("com.github.sbt" % "sbt-ci-release" % "1.11.2")
addSbtPlugin("com.github.sbt" % "sbt-coffeescript" % "2.1.0-M2")
addSbtPlugin("com.github.sbt" % "sbt-dynver" % "5.1.1")
addSbtPlugin("com.github.sbt" % "sbt-git" % "2.1.0")
addSbtPlugin("com.github.sbt" % "sbt-header" % "5.11.0")
addSbtPlugin("com.github.sbt" % "sbt-jacoco" % "3.6.0")
addSbtPlugin("com.github.sbt" % "sbt-java-formatter" % "0.11.0")
addSbtPlugin("com.github.sbt" % "sbt-javaagent" % "0.2.0")
addSbtPlugin("com.github.sbt" % "sbt-js-engine" % "1.4.0-M3")
addSbtPlugin("com.github.sbt" % "sbt-less" % "2.1.0-M1")
addSbtPlugin("com.github.sbt" % "sbt-license-report" % "1.9.0")
addSbtPlugin("com.github.sbt" % "sbt-maven-plugin" % "0.0.3")
addSbtPlugin("com.github.sbt" % "sbt-mocha" % "2.2.0-M2")
addSbtPlugin("com.github.sbt" % "sbt-native-packager" % "1.11.7")
addSbtPlugin("com.github.sbt" % "sbt-pgp" % "2.3.1")
addSbtPlugin("com.github.sbt" % "sbt-protobuf" % "0.8.3")
addSbtPlugin("com.github.sbt" % "sbt-release" % "1.4.0")
addSbtPlugin("com.github.sbt" % "sbt-unidoc" % "0.6.1")
addSbtPlugin("com.github.sbt" % "sbt-web" % "1.6.0-M3")
addSbtPlugin("com.github.sbt" % "sbt-web-build-base" % "2.1.0-M3")
addSbtPlugin("com.github.sbt" % "sbt2-compat" % "0.1.0")
addSbtPlugin("com.github.sbt.junit" % "sbt-jupiter-interface" % "0.18.0")
addSbtPlugin("com.github.scalaprops" % "sbt-scalaprops" % "0.5.2")
addSbtPlugin("com.github.xuwei-k" % "sbt-class-diagram" % "0.3.1")
addSbtPlugin("com.github.xuwei-k" % "sbt-conflict-classes" % "0.1.2")
addSbtPlugin("com.github.xuwei-k" % "sbt-jol" % "0.3.2")
addSbtPlugin("com.github.xuwei-k" % "sbt-jshell" % "0.1.5")
addSbtPlugin("com.github.xuwei-k" % "sbt-root-aggregate" % "0.1.0")
addSbtPlugin("com.github.xuwei-k" % "scalafix-check" % "0.1.0")
addSbtPlugin("com.github.xuwei-k" % "scalafix-rule-resource-gen" % "0.1.2")
addSbtPlugin("com.github.xuwei-k" % "test-times-plugin" % "0.1.1")
addSbtPlugin("com.github.xuwei-k" % "unused-code-plugin" % "0.5.5")
addSbtPlugin("com.github.xuwei-k" % "warning-diff-scalafix-plugin" % "0.6.6")
addSbtPlugin("com.indoorvivants" % "sbt-commandmatrix" % "0.1.0")
addSbtPlugin("com.julianpeeters" % "sbt-avrohugger" % "2.16.3")
addSbtPlugin("com.performantdata" % "sbt-sliquibase" % "0.3.0")
addSbtPlugin("com.siriusxm" % "sbt-snapshot4s" % "0.2.8")
addSbtPlugin("com.softwaremill.sttp.tapir" % "sbt-openapi-codegen" % "1.13.15")
addSbtPlugin("com.typesafe" % "sbt-mima-plugin" % "1.1.5")
addSbtPlugin("dev.cheleb" % "sbt-fullstack-js" % "0.7.0")
addSbtPlugin("dev.cheleb" % "sbt-plantuml" % "0.3.2")
addSbtPlugin("io.github.matejcerny" % "sbt-config" % "0.5.2")
addSbtPlugin("io.github.polentino" % "sbt-redacted" % "1.1.1")
addSbtPlugin("io.github.sbt-dao-generator" % "sbt-dao-generator" % "2.1.0")
addSbtPlugin("io.github.sbt-doctest" % "sbt-doctest" % "0.12.4")
addSbtPlugin("io.github.sbt-teavm" % "sbt-teavm" % "0.4.1")
addSbtPlugin("io.github.scalauser12" % "sbt-release-io" % "0.9.1")
addSbtPlugin("io.github.scalauser12" % "sbt-release-io-monorepo" % "0.9.1")
addSbtPlugin("io.h8.sbt" % "sbt-dependencies" % "1.0.4")
addSbtPlugin("io.stryker-mutator" % "sbt-stryker4s" % "0.20.3")
addSbtPlugin("me.limansky" % "sbt-test-it" % "0.3.0")
addSbtPlugin("me.seroperson" % "sbt-live-reload" % "0.1.1")
addSbtPlugin("net.nmoncho" % "sbt-dependency-check" % "1.8.5")
addSbtPlugin("net.sf.ij-plugins" % "sbt-imagej" % "2.2.1")
addSbtPlugin("nl.gn0s1s" % "sbt-banner" % "0.0.2")
addSbtPlugin("nl.gn0s1s" % "sbt-dotenv" % "3.2.0")
addSbtPlugin("nl.gn0s1s" % "sbt-jackson-version-check" % "0.0.7")
addSbtPlugin("nl.gn0s1s" % "sbt-license-check" % "0.0.6")
addSbtPlugin("nl.gn0s1s" % "sbt-pekko-version-check" % "0.0.8")
addSbtPlugin("org.foundweekends.conscript" % "sbt-conscript" % "0.5.9")
addSbtPlugin("org.foundweekends.giter8" % "sbt-giter8" % "0.18.0")
addSbtPlugin("org.foundweekends.giter8" % "sbt-giter8-scaffold" % "0.18.0")
addSbtPlugin("org.jetbrains.scala" % "sbt-idea-shell" % "2025.3.0")
addSbtPlugin("org.jetbrains.scala" % "sbt-structure-extractor" % "2026.1.3")
addSbtPlugin("org.latestbit" % "sbt-gcs-plugin" % "2.0.0-RC1")
addSbtPlugin("org.playframework" % "play-docs-sbt-plugin" % "3.1.0-M5")
addSbtPlugin("org.playframework" % "sbt-plugin" % "3.1.0-M5")
addSbtPlugin("org.playframework" % "sbt-scripted-tools" % "3.1.0-M5")
addSbtPlugin("org.playframework.twirl" % "sbt-twirl" % "2.1.0-M9")
addSbtPlugin("org.scala-sbt" % "sbt-contraband" % "0.8.0")
addSbtPlugin("org.scalameta" % "sbt-mdoc" % "2.8.2")
addSbtPlugin("org.scalameta" % "sbt-metals" % "1.6.7")
addSbtPlugin("org.scalameta" % "sbt-munit" % "1.2.4")
addSbtPlugin("org.scalameta" % "sbt-native-image" % "0.4.0")
addSbtPlugin("org.scalameta" % "sbt-scalafmt" % "2.5.6")
addSbtPlugin("org.scalaxb" % "sbt-scalaxb" % "1.12.4")
addSbtPlugin("org.scoverage" % "sbt-scoverage" % "2.4.4")
addSbtPlugin("org.wartremover" % "sbt-wartremover" % "3.5.6")
addSbtPlugin("org.wartremover" % "sbt-wartremover-contrib" % "2.4.3")
addSbtPlugin("pl.project13.scala" % "sbt-jmh" % "0.4.8")
